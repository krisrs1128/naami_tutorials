...
\begin{frame}
  ...
  \item Chain rule can be used again to simplify computation
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Loss using final $K^{th}$ layer representation,
    ...
    \end{align*}
  \item How does the loss change when you perturb $W_{k}$ a little?
  \item Derivative of loss with respect to final layer weights
    \begin{align*}
      D_{W_K}\left(\ell \circ f^{K}\left(h_{k - 1}\right)\right) &=
      D_{W_{k}}\left(\ell\circ \sigma\left(W_k h_{k - 1}\right)\right)
    \end{align*}
  \item This is just like logistic regression!!
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Inductive Hypothesis}
  \begin{itemize}
  \item Loss using final ($K^{th}$) layer representation,
    \begin{align*}
    \ell\left(y_i, \sigma\left(W_{K + 1} h_{K}\right)
    \end{align*}
  \item First consider derivative of loss with respect to final layer weights
    %% \begin{align*}
