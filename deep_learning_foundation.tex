...
\begin{frame}
  ...
  \item Derivative of composition $\rightarrow$ multiplication of derivatives
    \begin{align*}
      D\left(f^{K} \circ f^{K - 1} \dots \circ f^{1}\right)\left(x\right) &= \left[\prod_{k = 2}^{K} D f^{k}\left(f^{k - 1}\left(x\right)\right)\right]D f^1\left(x\right)
    \end{align*}
  \end{itemize}
  \begin{figure}[ht]
    \centering
...
\begin{frame}
  ...
  \item Representation at layer $k$,
    \begin{align*}
    h_{k} = f^k\left(h_{k - 1}\right) \stackrel{\text{defined}}{=} \sigma\left(W_k h_{k - 1}\right)
    \end{align*}
    \begin{itemize}
    \item Convention that $h_0 \stackrel{\text{defined}}{=} x_i$
    \end{itemize}
  \item When you change $W_k$ a little, it changes all the upstream $h_k, h_{k +
...
\begin{frame}
  ...
  \begin{itemize}
  \item Loss using final $K^{th}$ layer representation,
    \begin{align*}
    \ell\left(y_i, h_K\right) &= \ell\left(y_i, f^{k}\left(h_{K - 1}\right)\right) \\
    &= \ell\left(y_i, \sigma\left(W_{K}h_{k - 1}\right)\right)
    \end{align*}
  \item How does the loss change when you perturb $W_{k}$ or $h_{k - 1}$ a little?
  \item Derivative of loss with respect to final layer weights
    \begin{align*}
      D\left(\ell \circ f^{K}\left(h_{k - 1}\right)\right) &=
      D\left(\ell\circ \sigma\left(W_k h_{k - 1}\right)\right)
    \end{align*}
  \item Can compute derivatives using chain rule, just like in logistic
    regression
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Hypothesis: Suppose we know how to compute derivatives for all $f_{K},
    f_{K - 1}, \dots, f_{k + 1}$ with respect to their inputs
  \item Step: Then derivative of $\ell$ with respect to input $h_k - 1$
    \begin{align*}
      D\left(\ell \circ f^{K} \circ \dots f^{k + 1} \circ f^{k}\right) &= D\ell\left(h^{K}\right)
      Df^{K}\left(h_{K - 1}\right) \dots Df^{k + 1}\left(h_{k}\right) Df^{k}\left(h_{k - 1}\right)
    \end{align*}
  \item Since $Df^{k}\left(h_{k - 1}\right) = D\left(\sigma\left(W_{k}h_{k
    -1}\right)\right)$ is straightforwards, induction is complete
    %% \begin{align*}
    %%   D_{W_k}}\left(\ell \circ f^{k}\left(h_k - 1\right)\right)
    %% \end{align*}
  \end{itemize}
\end{frame}
